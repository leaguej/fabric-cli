...
type Printer interface {
	...
	PrintChaincodeData(ccdata *ChaincodeData)
...
func (p *BlockPrinter) PrintChaincodeData(ccData *ChaincodeData) {
	if p.Formatter == nil {
		fmt.Printf("%s\n", ccData)
		return
	}
...
func (p *BlockPrinter) PrintCDSData(cdsData *CDSData) {
...
func (p *BlockPrinter) PrintConfigUpdateEnvelope(envelope *fabriccmn.ConfigUpdateEnvelope) {
	...
	configUpdate, err := UnmarshalConfigUpdate(envelope.ConfigUpdate)
	if err != nil {
		panic(err)
	}
...
func (p *BlockPrinter) PrintTxReadWriteSet(txRWSet *TxRwSet) {
	p.Array("NsRWs")
...
func (p *BlockPrinter) PrintNsReadWriteSet(nsRWSet *NsRwSet) {
...
func (p *BlockPrinter) PrintBlockMetadata(blockMetaData *fabriccmn.BlockMetadata) {
	...
	p.PrintTransactionsFilterMetadata(TxValidationFlags(blockMetaData.Metadata[fabriccmn.BlockMetadataIndex_TRANSACTIONS_FILTER]))
...
func (p *BlockPrinter) PrintTransactionsFilterMetadata(txFilter TxValidationFlags) {
